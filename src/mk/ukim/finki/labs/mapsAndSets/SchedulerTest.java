...
class Scheduler<T> {
    ...
    public void add(Date d, T t) {
        this.timestamps.put(d, t);
    }

    public boolean remove(Date d) {
        return this.timestamps.remove(d) != null;
    }

    public T next() {
        return this.timestamps.ceilingEntry(Date.from(Instant.now())).getValue();
    }

    public T last() {
        return this.timestamps.floorEntry(Date.from(Instant.now())).getValue();
    }

    public ArrayList<T> getAll(Date begin, Date end) {
        return (ArrayList<T>) this.timestamps.entrySet().stream()
                .filter(k -> k.getKey().getTime() > begin.getTime() && k.getKey().getTime() < end.getTime())
                .map(Map.Entry::getValue)
                .collect(Collectors.toList());
    }

    public T getFirst() {
        return this.timestamps.entrySet().stream()
                .reduce((a, b) -> a.getKey().getTime() < b.getKey().getTime() ? a : b).get().getValue();
    }
