...
class LogSystem<T extends ILog> {
    ...
    void printResults() {
        ...
        LogProcessor<T> firstLogProcessor = log -> (ArrayList<T>) log.stream().filter(l -> l.getType().equals("INFO")).collect(Collectors.toList());

        LogProcessor<T> secondLogProcessor = log -> (ArrayList<T>) log.stream().filter(l -> l.getMessage().length() < 100).collect(Collectors.toList());

        LogProcessor<T> thirdLogProcessor = log -> (ArrayList<T>) log.stream().sorted(Comparator.comparing(l -> l.getTimestamp())).collect(Collectors.toList());
