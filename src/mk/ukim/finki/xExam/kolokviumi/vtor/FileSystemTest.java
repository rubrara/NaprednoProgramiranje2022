...
class FileSystem {
    ...
    public void addFile(char folder, String name, int size, LocalDateTime createdAt) {
        ...
        filesByFolder.computeIfPresent(folder, (k, v) -> {
            v.add(new File(name, size, createdAt));
            return v;
        });
    }
    ...
    public List<File> findAllHiddenFilesWithSizeLessThen(int size) {
        ...
        return filesByFolder.values().stream().flatMap(Collection::stream)
    ...
    public int totalSizeOfFilesFromFolders(List<Character> folders) {
        ...
        return folders.stream().mapToInt(f -> filesByFolder.get(f).stream().mapToInt(File::getSize).sum()).sum();
...
class File implements Comparable<File> {
    ...
    public String toMonthDay() {
        return String.format("%s-%d", createdAt.getMonth(), getDayOfMonth());
    }
    ...
    public int compareTo(File o) {
        return Comparator.comparing(File::getCreatedAt).thenComparing(File::getSize).thenComparing(File::getName).compare(this, o);
    }
}
